...
- 不在 CodexClaw 主仓库中保存 Homebrew tap 写入凭据；tap 推送由 tap 仓库自己的权限或本地维护者权限完成。
- 所有 GitHub Actions 都固定到不可变的 commit SHA，而不是漂移的版本标签。

## 当前对应关系

- `scripts/check-action-pinning.sh`：如果 workflow 里出现浮动 tag 而不是 SHA，直接让 CI 失败。
- `.github/workflows/homebrew.yml`：用 tag 源码包生成 formula 并打印，避免主仓库保存跨仓库写入 token。

## 限制和前提

- 当前没有单独的 dependency review、OSV、SBOM 或 provenance workflow；第一版先避免为了模板感堆过重的 CI/CD。
- Homebrew tap 写入权限需要单独配置。当前不使用 `HOMEBREW_TAP_TOKEN`；如果平台支持 GitHub-to-GitHub tap 写入的 trusted publishing，再切到 OIDC/trust 模式。
